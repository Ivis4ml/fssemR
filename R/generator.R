...
##' @importFrom qtl sim.map sim.cross pull.geno find.markerpos sim.geno makeqtl
##' @export
randomFSSEMdata4Cor = function(n, p, k, sparse = 0.1, df = 0.2, sigma2 = 0.01, u = 5, type = c("DG", "ER"), dag = TRUE,
                           coef = c(0.2, 0.4), nhub = 2, r = 0.5) {
  mincoef = coef[1]
  maxcoef = coef[2]
  type = match.arg(type)
  DG = function() {
  ...
  Sk = QTL$Sk
  E = vector("list", 2)
  E[[1]] = t(rmvnorm(n[1], mean = rep(0, p), sigma = diag(p)))
  E[[2]] = t(rmvnorm(n[2], mean = rep(0, p), sigma = diag(p)))
  for (i in 1:n[2]) {
    ## E[[2]][,i] = corVar(E[[1]][,i], E[[2]][,i], r)
    E[[2]][,i] = r * E[[1]][,i] + sqrt(1 - r**2) * E[[2]][,i]
  }
  mu = lapply(1:2, function(i) {
    tcrossprod(rnorm(p, 0, u), rep(1, n[i]))
  })
  Y = lapply(1:2, function(i) {
    solve(diag(p) - B[[i]]) %*% (F %*% X + mu[[i]] + sqrt(sigma2) * E[[i]])
  })
  names = paste0("g", seq(1, p))
  names(QTL$marker) = names
  names(QTL$eQTLs)  = names
